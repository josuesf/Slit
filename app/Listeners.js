...
export function registerAppListener(navigation) {
  FCM.on(FCMEvent.Notification, notif => {
    ...
    if (notif.custom_notification) {
      let datos_notif = JSON.parse(notif.custom_notification)
      realm.write(() => {
        realm.create('Chats', {
          id_chat: datos_notif.id_usuario,
          usuario: datos_notif.id_usuario,
          token: '',
          ultimo_mensaje: datos_notif.body,
          timestamp: datos_notif.timestamp,
          avatar: '',
          estado_mensaje: 'recibido',
          rol: 'receptor',
          tipo_mensaje: 'texto',
          id_mensaje: datos_notif.id_mensaje
        }, true);
        realm.create('ChatList', {
          id_mensaje: datos_notif.id_mensaje,
          mensaje: datos_notif.body,
          tipo_mensaje: datos_notif.tipo_mensaje,
          timestamp: datos_notif.timestamp,
          estado_mensaje: 'recibido',
          id_chat: datos_notif.id_usuario,
          id_usuario: datos_notif.id_usuario
        }, true);
      });
    }else{
      realm.write(() => {
        realm.create('Chats', {
          id_chat: notif.id_usuario,
          usuario: notif.id_usuario,
          token: '',
          ultimo_mensaje: notif.body,
        ...
        realm.create('ChatList', {
          id_mensaje: notif.id_mensaje,
          mensaje: notif.body,
          tipo_mensaje: notif.tipo_mensaje,
          timestamp: notif.timestamp,
          estado_mensaje: 'recibido',
          id_chat: notif.id_usuario,
    ...
    if (notif.opened_from_tray) {
      if (notif.targetScreen === 'detail') {
        setTimeout(() => {
          navigation.navigate('chat', { usuario: notif.id_usuario })
        }, 0)
      }
      setTimeout(() => {
